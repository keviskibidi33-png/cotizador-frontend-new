...
export function QuoteBuilderPage() {
  ...
  const handleLoadPlantilla = async (plantillaId: string) => {
    try {
      ...
      console.log('Condiciones parseadas (pre):', plantilla.condiciones_ids);

      // Parsear condiciones_ids de PostgreSQL array string a JS array
      let condicionesIds: string[] = [];
      const rawCids = plantilla.condiciones_ids;

      if (typeof rawCids === 'string') {
        if (rawCids.startsWith('{')) {
          condicionesIds = rawCids.slice(1, -1).split(',').map((s: string) => s.trim()).filter(Boolean);
        } else if (rawCids.length > 0) {
          condicionesIds = [rawCids];
        }
      } else if (Array.isArray(rawCids)) {
        condicionesIds = rawCids.filter(Boolean).map(String);
      }

      console.log('Condiciones parseadas (post):', condicionesIds);
    ...
    async function loadQuote() {
      try {
        ...
        if (data.condiciones_ids) {
          let cIds = data.condiciones_ids;
          let parsedIds: string[] = [];

          if (typeof cIds === 'string') {
            if (cIds.startsWith('{')) {
              parsedIds = cIds.replace('{', '').replace('}', '').split(',').map((s: string) => s.trim()).filter(Boolean);
            } else if (cIds.length > 0) {
              parsedIds = [cIds];
            }
          } else if (Array.isArray(cIds)) {
            parsedIds = cIds.filter(Boolean).map(String);
          }

          console.log('LoadQuote: parsed IDS:', parsedIds);
          setSelectedCondiciones(parsedIds);
        }
        ...
        // Update UI search fields
        setClienteSearch(data.cliente || '');
        setProyectoSearch(data.proyecto || '');

        // Reconstruct selected objects to ensure persistence
        if (data.cliente_id) {
          setSelectedCliente({
            id: data.cliente_id,
            nombre: data.cliente || '',
  ...
  useEffect(() => {
    // Si estamos editando, NO cargar siguiente correlativo
    if (urlParams.quote_id) return;
    ...
    loadNext();
  }, [apiBaseUrl, header.fecha_emision, urlParams.quote_id]);
  ...
  useEffect(() => {
    if (proyectoSearch.length >= 2 && !selectedProyecto) {
      // Allow searching projects even if no client is selected
      searchProyectos(selectedCliente?.id || undefined, proyectoSearch);
  ...
  async function onExport() {
    ...
    try {
      const payload: QuotePayload = {
        ...
        correo_vendedor: (isEditMode && header.correo_vendedor) ? header.correo_vendedor : (urlParams.email || ''),
        ...
        // Use selected object ID if available, otherwise fallback to header (loaded from DB)
        proyecto_id: selectedProyecto?.id || header.proyecto_id,
        cliente_id: selectedCliente?.id || header.cliente_id,
      };
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="rounded-lg border border-border bg-white p-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="space-y-2 relative">
              ...
              />
              {showProyectoDropdown && (proyectos.length > 0 || (selectedCliente && proyectoSearch.length >= 2)) && (
                <div className="absolute z-10 mt-1 w-full bg-white border border-slate-200 rounded-md shadow-lg max-h-48 overflow-auto">
                  {proyectos.map(p => (
                    <div key={p.id} onClick={() => selectProyecto(p)} className="px-3 py-2 hover:bg-slate-100 cursor-pointer text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        {p.cliente_nombre && !selectedCliente && <div className="text-xs text-blue-500">{p.cliente_nombre}</div>}
                      </div>
                    </div>
                  ))}
                  ...
                  {selectedCliente ? (
                    <div onClick={() => { setShowNewProyectoModal(true); setShowProyectoDropdown(false); }} className="px-3 py-2 hover:bg-blue-50 cursor-pointer text-sm text-blue-600 border-t font-medium sticky bottom-0 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                      <Plus className="inline h-3 w-3 mr-1" /> Crear nuevo proyecto para {selectedCliente.nombre.slice(0, 20)}...
                    </div>
                  ) : (
                    <div className="px-3 py-2 text-sm text-slate-400 border-t font-medium italic sticky bottom-0 bg-white">
                      Selecciona un cliente para crear proyectos
                    </div>
                  )}
                </div>
              )}
      ...
      {
        notification?.show && (
          <div className={`fixed top-4 right-4 z-[70] animate-in slide-in-from-top-2 duration-300 ${notification.type === 'success' ? 'bg-green-500' : 'bg-red-500'
                  ...
                  <div className="space-y-3 p-6">
                    {plantillas.filter(p => {
                      ...
                      return (
                        <div
                          key={plantilla.id}
                          className={`border rounded-lg transition-all relative ${isExpanded
